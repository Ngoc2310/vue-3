...
export default {
  ...
  methods: {
    init() {
      ...
      if (
        ...
        exportBtn.setAttribute(
          "class",
          `oc-export-btn${this.exportBtnClass ? ` ${this.exportBtnClass}` : ""}`
        );
    ...
    hideSiblings(node, direction) {
      const nodeContainer = this._closest(
        node,
        (el) => el.nodeName === "TABLE"
      ).parentNode;
      ...
      animatedNodes.forEach((_node) => {
        const temp = this._closest(_node, (el) =>
          el.classList.contains("nodes")
      ...
      this._one(animatedNodes[0], "transitionend", () => {
        lines.forEach((line) => {
    ...
    _clickBottomEdge(event) {
      ...
      if (childrenState.exist) {
        const temp = this._closest(node, (el) => el.nodeName === "TR")
    ...
    _buildSiblingNode(nodeChart, nodeData, callback) {
      ...
      const existingSibligCount =
        nodeChart.parentNode.nodeName === "TD"
          ? this._closest(nodeChart, (el) => el.nodeName === "TR").children
              .length
          : 1;
    ...
    _createNode(nodeData, level) {
      ...
      const rootNode = {
        ...
        render: (h) => {
          const { tag, data, children } = this.$slots.default({
            nodeData,
          });
          return h(tag, data, children);
        },
      };
      const vmNode = new createApp(rootNode);
      vmNode.mount("");
    ...
    _clickExportButton() {
      ...
      const sourceChart = chartContainer.querySelector(
        ".orgchart:not(.hidden)"
      );
